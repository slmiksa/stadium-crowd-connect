  ...
  return <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-zinc-900 rounded-lg p-8 border border-zinc-800">
          ...
          <div className="mb-6">
            <Button type="button" variant="outline" className="w-full bg-white text-black hover:bg-white hover:text-black border-gray-300" onClick={handleGoogleSignIn} disabled={isLoading}>
          ...
          <div className="mb-6 text-center">
            ...
            <div className="flex justify-center gap-4">
                <Button type="button" variant="outline" onClick={() => handleInstallGuideClick('android')} className="w-full text-slate-50">
                    <Bot className={isRTL ? "ml-2 h-5 w-5" : "mr-2 h-5 w-5"} />
                    Android
                </Button>
                <Button type="button" variant="outline" onClick={() => handleInstallGuideClick('ios')} className="w-full text-slate-50">
          ...
          <Tabs defaultValue="login" className="w-full">
            ...
            <TabsContent value="login">
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <Button type="submit" disabled={isLoading} className="w-full text-slate-50">
                  {isLoading ? <>
      ...
      <AlertDialog open={isInstructionDialogOpen} onOpenChange={setIsInstructionDialogOpen}>
        <AlertDialogContent dir={isRTL ? 'rtl' : 'ltr'}>
            <AlertDialogHeader>
                <AlertDialogTitle>
                    {platform === 'android' ? isRTL ? 'تثبيت التطبيق على أندرويد' : 'Install App on Android' : isRTL ? 'تثبيت التطبيق على iOS' : 'Install App on iOS'}
                </AlertDialogTitle>
                <AlertDialogDescription asChild>
                  {platform === 'android' ? <div className={`space-y-3 pt-2 ${isRTL ? 'text-right' : 'text-left'}`}>
                      ...
                      </div> : <div className={`space-y-3 pt-2 ${isRTL ? 'text-right' : 'text-left'}`}>
                          <p>{isRTL ? 'لإضافة التطبيق إلى شاشتك الرئيسية، اتبع الخطوات التالية:' : 'To add the app to your home screen, follow these steps:'}</p>
                          <ol className="list-decimal list-inside space-y-2">
