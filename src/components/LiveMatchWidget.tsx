...
const LiveMatchWidget: React.FC<LiveMatchWidgetProps> = ({
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (updatedMatch) {
          ...
          const {
            error: updateError
          } = await supabase.from('room_live_matches').update({
  ...
  const fetchLiveMatch = useCallback(async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('room_live_matches').select('*').eq('room_id', roomId).eq('is_active', true).maybeSingle();
      if (error) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`room-live-match-${roomId}`).on('postgres_changes', {
      event: '*',
      schema: 'public',
  ...
  const removeLiveMatch = async () => {
    try {
      ...
      const {
        error
      } = await supabase.from('room_live_matches').delete().eq('room_id', roomId);
      if (!error) {
        setMatchData(null);
        onRemove?.();
  ...
  if (isLoading) {
    return <div className="bg-gradient-to-r from-green-500/20 to-blue-500/20 border border-green-500/30 rounded-lg p-3 animate-pulse mb-2">
  ...
  return <div className="bg-gradient-to-r from-green-500/20 to-blue-500/20 border border-green-500/30 p-3 mb-2 py-0 rounded-3xl mx-[16px]">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center space-x-2 space-x-reverse">
          ...
          {matchData.status === 'live' && <span className="text-xs text-gray-500">
              (تحديث كل {updateInterval} دقيقة)
            </span>}
        </div>
        {isOwnerOrModerator && <Button onClick={removeLiveMatch} size="sm" variant="ghost" className="h-6 w-6 p-0 text-gray-400 hover:text-red-400" title="إيقاف النقل المباشر (للمشرفين فقط)">
            <X size={12} />
          </Button>}
      </div>

      {/* منطقة المباراة القابلة للضغط */}
      <div onClick={handleMatchClick} className="cursor-pointer hover:bg-white/5 rounded-lg p-2 transition-colors" title="اضغط لعرض تفاصيل المباراة">
        <div className="flex items-center justify-between">
          {/* الفريق المحلي */}
          <div className="flex items-center space-x-2 space-x-reverse flex-1">
            {matchData.homeLogo && <img src={matchData.homeLogo} alt={matchData.homeTeam} className="w-6 h-6 object-contain" />}
          ...
          <div className="flex items-center space-x-2 space-x-reverse flex-1 justify-end">
            ...
            {matchData.awayLogo && <img src={matchData.awayLogo} alt={matchData.awayTeam} className="w-6 h-6 object-contain" />}
          </div>
        </div>
      </div>

      {matchData.status === 'live' && <div className="flex items-center justify-center mt-2 space-x-2 space-x-reverse">
